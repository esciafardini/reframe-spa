...
## The following instructions come from Re-Frame's "lein new re-frame" template.....
...
* [`src/novo_challenge/`](src/novo_challenge/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/novo_challenge/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `novo-challenge` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/novo_challenge/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns novo-challenge.example
  (:require [novo-challenge.config :as config])
